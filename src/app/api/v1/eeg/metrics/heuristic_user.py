"""
Apply Heuristic Metric to All EEG Image Evaluations of a User

This endpoint applies the EEG heuristic metric to all EEGImageEvaluation instances
associated with a specific user. It calculates the metric for each image,
...
@router.post(
    path="/heuristic/user/{user_id}/",
    summary="Apply heuristic metric to all EEG image evaluations of a user",
    description=(
        "Calculates the EEG heuristic metric for all EEGImageEvaluation instances "
        "associated with a specific user, updates each metric_heurist field, "
        "and returns a summary of the results."
    ),
)
async def apply_heuristic_metric_user(
    user_id: Annotated[int, FastAPIPath(gt=0)],
    db: Annotated[Session, Depends(get_session)],
) -> dict:
    """
    Applies heuristic metric to all EEG image evaluations for a specific user.
    ...
    """
    try:
        # Retrieve all image evaluations linked to this user
        image_evaluations = db.exec(
            select(EEGImageEvaluation).where(EEGImageEvaluation.user_id == user_id)
        ).all()

        if not image_evaluations:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No EEG image evaluations found for user ID {user_id}.",
            )
