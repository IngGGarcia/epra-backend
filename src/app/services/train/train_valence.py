"""
Valence Training Service

This module provides functionality for training a Gaussian Process Regression (GPR) model
to predict valence values from image features. The valence dimension represents the
pleasantness or unpleasantness of an emotional response.

The module uses normalized EEG data to train a GPR model that can predict valence
...
References:
    ...
    - Valence in Emotion Recognition: Bradley, M. M., & Lang, P. J. (1994). Measuring emotion:
...
def train_valence_model(
    training_data: list[ImageTrainingData],
) -> GaussianProcessRegressor:
    """
    Train a GPR model for valence prediction using the provided training data.

    This function processes EEG valence data and image feature vectors to create
    a predictive model. The valence values are normalized to a standard range
    ...
    Args:
        training_data (list[ImageTrainingData]): List of training data objects containing:
            ...
            - eeg_valence: Raw EEG valence value
            - eeg_arousal: Raw EEG arousal value (not used in this function)

    Returns:
        GaussianProcessRegressor: Trained GPR model for valence prediction
    ...
    Examples:
        ...
        ...     )
        ... ]
        >>> model = train_valence_model(training_data)
    ...
    # Extract features and valence values
    X = [data.feature_vector for data in training_data]
    y = [data.eeg_valence for data in training_data]
    ...
    # Normalize valence values to standard range
    y = normalize_range(y)
